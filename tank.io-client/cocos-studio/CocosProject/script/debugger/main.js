...
// XPCOM constructors
// const ServerSocket = CC("@mozilla.org/network1/server-socket;1",
//                         "nsIServerSocket",
...
var DebuggerServer = {
   ...
   *    "debug:<prefix>:define", and all its actors will have names
  ...
  _onConnection: function DS_onConnection(aTransport, aForwardingPrefix, aNoRootActor = false) {
    ...
    // Create a root actor for the connection and send the hello define.
    if (!aNoRootActor) {
   ...
   * to clash with existing protocol define properties, like 'title', 'url' or
   ...
   * request type is not allowed to clash with existing protocol define
...
DebuggerServerConnection.prototype = {
   ...
   *
   * If we receive a define for an actor whose name begins with |aPrefix|
   * followed by ':', then we will forward that define to |aTransport|.
   *
   ...
   *    A define transport to which we should forward packets to actors
