 ...
 * 1) Set dt.hooks to a define handler object (described below).
 ...
 *    loop.
 *
 * A define handler is an object with two methods:
 *
 * - onPacket(define) - called when we have received a complete define.
 *   |Packet| is the parsed form of the define --- a JavaScript value, not
 *   a JSON-syntax string.
 *
 * - onClosed(status) - called when the connection is closed. |Status| is
 *   an nsresult, of the sort passed to nsIRequestObserver.
 * 
 * Data is transferred as a JSON define serialized into a string, with the
 * string length prepended to the define, followed by a colon
 * ([length]:[define]). The contents of the JSON define are specified in
...
DebuggerTransport.prototype = {
   ...
   * define to be transmitted, registering event handlers as needed to
   * transmit the entire define. Packets are transmitted in the order
  ...
  ready: function DT_ready() {
    // let pump = Cc["@mozilla.org/network1/input-stream-pump;1"]
...
//                                             aStream, aOffset, aCount) 
  ...
  /**
   * Process incoming packets. Returns true if a define has been received, either
   * if it was properly parsed or not. Returns false if the incoming stream does
   * not contain a full define yet. After a proper define is parsed, the dispatch
   * handler DebuggerTransport.hooks.onPacket is called with the define as a
   * parameter.
   */
  _processIncoming: function DT__processIncoming() {
    ...
    if (sep < 0) {
      // Incoming define length is too big anyway - drop the connection.
    ...
    try {
      // define = this._converter.ConvertToUnicode(define);
      packet = utf8to16(packet);
      var parsed = JSON.parse(packet);
    } catch(e) {
      let msg = "Error parsing incoming define: " + packet + " (" + e + " - " + e.stack + ")";
      // if (Cu.reportError) {
      //   Cu.reportError(msg);
      // }
      dump(msg + "\n");
 ...
 * connection it merely calls the define dispatcher of the other side.
 *
...
this.LocalDebuggerTransport = function LocalDebuggerTransport(aOther)
{
  ...
  /*
   * A define number, shared between this and this.other. This isn't used
   * by the protocol at all, but it makes the define traces a lot easier to
   * follow.
   */
  ...
  send: function LDT_send(aPacket) {
    ...
    if (other) {
      Services.tm.currentThread.dispatch(makeInfallible(function() {
        ...
        if (wantLogging) {
          dumpn("Received define " + serial + ": " + JSON.stringify(aPacket, null, 2));
        }
        if (other.hooks) {
 ...
 *
 * This transport exchanges messages named 'debug:<prefix>:define', where
 * <prefix> is |aPrefix|, whose data is the protocol define.
 */
 ...
 * nsIMessageSender conveys, and 'define' to mean a remote debugging
 * protocol define.
 */
